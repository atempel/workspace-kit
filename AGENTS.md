...
**Git integration layer (`core/git.js`):** file-state tracking, the safe-edit check, a plain-language change summary and a templated commit message (docs/specs/git-integration-layer.md → #79). Builds on the inspection index rather than re-walking the folder. Model-agnostic is a product rule here, not a preference: every string it emits is a deterministic template over file names and diff counts, and a test asserts the module makes no outbound call. Git is driven with explicit argument arrays, never a shell string. Worktrees are managed here too (`worktree list|add|remove`): by convention at `<workspace>/.worktrees/<name>` on a same-named branch, with `--path`/`--branch` overriding either, and the ignore rule added automatically so a workspace never reports its own worktrees as stray files. Run `node bin/workspace-kit.js status [dir]` or `worktree list [dir]`. To test: `npm run test:git`. **Local git only, on purpose** (see DECISIONS.md, 2026-07-29): no push, no PR, no remote — a test enforces it. The PR flow and hosting-provider scope are a separate, later step.
...
- `reports/index.html`, `reports/plans/`, `reports/decisions.html`, `reports/tasks.html`, `reports/sessions.html` — **generated** by `npm run report`. Regenerate them; don't edit them.
- Anything else (one-off analyses, review write-ups) — hand-authored, and should say so at the top, including its date and which agent wrote it. House rules in `reports/README.md`.

When the owner asks for a report, write it here as HTML rather than answering only in the chat — summarize the headline in the chat, but the file is the deliverable. Markdown stays the source of truth for documentation; `reports/` is the human-friendly rendering of it, never a replacement.
...
- reports/ — HTML rendering of the docs for humans: generated by `npm run report`, plus hand-authored one-offs
