...
## Open Questions
- **(Resolved 2026-07-22, see DECISIONS.md)** Shared generation logic: `core/generator.js` is the shared generation engine — plain Node-compatible JS (UMD-lite: `module.exports` for Node, `window.WorkspaceKitCore` for `<script>`), zero dependencies, no build step. The CLI is expected to build directly on top of it via `require('../core/generator.js')`. Note the resolution's scope: it's a **copy-derived seed**, not a refactor-in-place — `src/workspace-kit.html` keeps its own separate inline copy for now (not wired to the module), to preserve its use as a standalone artifact pasted into Claude.ai. See DECISIONS.md, "2026-07-22 — Shared generation module seeded standalone, HTML artifact deferred", for the full reasoning and the manual-backport policy that follows from it.
