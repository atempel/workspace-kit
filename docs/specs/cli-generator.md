...
## Open Questions
- **(Resolved 2026-07-21, see DECISIONS.md)** Shared generation logic: the CLI, Web App, and HTML artifact will share one centralized generation engine, not independent copies — required so Templates (docs/specs/templates-feature.md) behave the same way regardless of which surface generated or ran them. Still open: the exact technical shape of a module includable both via `<script>` (HTML's no-build-step constraint) and via Node `require`/`import` (CLI/Web App) — see the DECISIONS.md entry's engineering follow-up.
