...
Status: **P0 implemented 2026-07-29** as `core/inspect.js` (tests: `npm run test:inspect`, 18 cases mapped 1:1 onto the P0 acceptance criteria below). Issue: [#77](https://github.com/atempel/workspace-kit/issues/77). Source: Notion page "Notas de Áudio 26/07" (2026-07-26); scoped out of docs/specs/local-web-app.md's (→ #29) "open an existing workspace" P0 bullet into its own foundational spec.
...
**Nice-to-Have (P1)**
- [x] **Done 2026-07-29.** Ignore-pattern support: `.git/`/`node_modules/` are skipped, the workspace's own `.gitignore` is honoured (covering the `localOnly` folder convention), and a directory carrying its own `.git` is treated as a separate repository. This turned out to be a correctness requirement rather than a nicety — see DECISIONS.md, 2026-07-29.
- [x] **Done 2026-07-29** in its minimal form: `node core/inspect.js <folder>` dumps the index as JSON. The real `workspace-kit inspect` command belongs to the CLI (#20) and should call `inspect()` directly.
...
## Open Questions
- **(Resolved 2026-07-29 in implementation, see DECISIONS.md)** Estimated token counting uses the **chars ÷ 4 heuristic**, swappable via an `estimateTokens` option. A real tokenizer would mean a runtime dependency, breaking the very convention this module mirrors, and docs/specs/workspace-health-check.md's P0 already assumed chars ÷ 4 — picking anything else would have split the two. A pluggable tokenizer stays a P2.
- **(Implemented as proposed 2026-07-29; still open for the Markdown Orchestrator to confirm)** One instruction unit = one markdown heading section, plus a preamble unit for content before the first heading (a generated `CLAUDE.md` opens with a bare `@AGENTS.md` import, which is itself an instruction). IDs are `path#heading-slug`. The Markdown Orchestrator still owns the definitive unit; `parseInstructions` in `core/inspect.js` is the single place that changes when it lands.
- **(Resolved 2026-07-29 in implementation)** No `window.*` global — `core/inspect.js` is Node-only (`module.exports`). Reading an arbitrary folder needs `fs`, so there is no browser surface to serve.
