...
## Problem Statement
Getting a project into a working dev container today means hand-writing a Dockerfile and docker-compose.yml, deciding on a base image, and wiring up whatever tooling the kit needs — the same kind of one-time setup friction workspace-kit already solves for context files. This repository's own `Dockerfile`/`docker-compose.yml` (Node 20 + Claude Code CLI + Python, no DB/JVM) is a concrete, working example of exactly the kind of output this feature should be able to generate automatically, adapted to whatever kit the workspace is for.
...
- Adapt the generated Docker template to the chosen kit (the same 17 kits already driving folders/anchor files), so a data/ML workspace and a mobile workspace don't get the same generic container.
...
- **A full matrix of every possible tool/language per kit at launch** — v1 covers a reasonable default per kit (see Requirements); exhaustive tool coverage per kit is a P1/P2 expansion, not a blocking v1 requirement.
...
- As a developer working on a data/ML project, I want the generated container to include the tooling that kit actually needs (e.g. Python) rather than a generic empty container.
...
- Base Docker template reused from this repo's own `Dockerfile`/`docker-compose.yml`: Node 20 + Claude Code CLI, `git`/`curl`/`unzip`/`build-essential`, Python3/pip/venv available, single `dev` service, bind-mounted project folder, `.claude_global` volume for Claude Code config, `.gitignore` entry for it.
  - Acceptance: a workspace generated with no kit-specific additions produces a Dockerfile/docker-compose.yml equivalent to this repo's own (same base packages, same volume/service structure).
- Kit → Docker adaptation mapping: at least a first pass covering the 17 existing kits, adding/removing base packages or services per kit where it clearly matters (e.g. data/ML keeps the Python stack front-and-center; a kit with no clear extra runtime need just gets the base template unchanged).
  - Acceptance: at least 3 kits produce a visibly different Dockerfile/docker-compose.yml from the base (not just a cosmetic comment change), reflecting real tooling differences.
...
- Broader per-kit tooling matrix (e.g. JVM for kits that need it, GPU-aware base images for heavier ML work) — deliberately deferred past the P0 "at least 3 kits differ" bar.
...
## Success Metrics
**Leading:** whether `docker compose build && docker compose up` succeeds without manual edits immediately after generation, across the kits covered in P0 (target: 100% of covered kits build cleanly, verified the same way the CLI/Web App specs verify generation parity — an explicit pass, not spot-checks).
...
## Open Questions
- **(Product — Alexandre)** Exact per-kit mapping: which kits need which extra packages/services? The P0 bar (≥3 kits visibly differ) is deliberately low to unblock a first pass — the full mapping is better defined once this is being built, informed by `research/kits/`.
- **(Engineering)** Where does the kit → Docker mapping live technically — hardcoded alongside the existing kit → folders/anchor-file mapping, or a separate config? Reasonable default is alongside the existing mapping, for consistency, unless a reason emerges not to.
