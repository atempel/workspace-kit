...
- **(Engineering — needs reconciliation once docs/specs/cross-tool-session-log.md exists)** Whether an ingestion event should also produce an entry in the cross-tool session log, or whether the queue item's own Status/Ingested-into fields are the sole record. The source material explicitly warns that three overlapping "history of what happened" files would defeat the purpose of any of them — this needs a real answer once that sibling spec's shape is settled, not a guess here. Current lean: the queue item is the authoritative record for *that specific reference's* lifecycle; the session log, if adopted, is a per-session narrative that may mention "ingested queue item X" in passing but isn't required to for the queue to do its job.
