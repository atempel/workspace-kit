...
- **Design-system-level polish** — MVP can reuse the HTML artifact's existing visual identity (terminal palette, JetBrains Mono/Inter) functionally rather than investing in new UI work; visual polish is a fast-follow once the mechanics work.
...
- "Open an existing workspace" flow: point the app at a folder previously generated by workspace-kit (or any folder following the same conventions) and see a read-only summary — which of PROJECT/DECISIONS/CONTEXT/TASKS/CLAUDE/AGENTS/etc. exist, project type, last-modified dates.
  - Acceptance: opening a real generated workspace correctly lists its actual file set; opening a folder with none of the expected files shows a clear "not a workspace-kit workspace" state rather than erroring.
...
## Open Questions
- **(Engineering — blocking, shared with the CLI spec)** Same generation-logic-sharing question: does the Web App, CLI, and HTML artifact share one generation module, or do they each carry their own copy of the templates? This should be answered once, for all three surfaces together, not per-spec.
