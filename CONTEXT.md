...
- "Kit" (formerly "project type") = one of the 17 built-in folder/anchor-file presets; built-ins are "system kits". Renamed 2026-07-26 (see DECISIONS.md) to avoid colliding with the separate, user-authored Templates feature; rename executed across code/UI 2026-07-29 (`KIT_CONFIG`, the `kit` input key, the form's `Kit` label). **Boundary:** the rename covers workspace//kit's *own* surface only — the text it *generates* still says "Project type" / "Tipo de projeto" (PROJECT.md heading, starter prompt), because that text belongs to the user's workspace vocabulary, not the product's (see DECISIONS.md, 2026-07-29).
